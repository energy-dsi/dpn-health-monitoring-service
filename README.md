...
The DPN Health Monitoring Service provides the observability backend infrastructure — logs, traces, and metrics collection, storage, and visualisation — for DPN Data Pipelines. It is one of several open-source components underpinning DSI's Data Preparation Node (DPN), a framework enabling secure, trusted data-sharing across organisations.

> **Note:** This repository contains no application code. Application services (producers/consumers) live in the [`dpn-data-pipelines`](https://github.com/energy-dsi/dpn-data-pipelines) repository. This repository is purely the observability stack that those services emit telemetry to.
...
- **Kubernetes-native deployment** — Helm charts for every component, with an Nginx reverse proxy for observability endpoints.
